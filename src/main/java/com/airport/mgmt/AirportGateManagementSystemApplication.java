...
public class AirportGateManagementSystemApplication {
	...
	public AirportGateManagementSystemApplication(GateRepository gateRepository, FlightRepository flightRepository, AccountRepository accountRepository) {
	...
	ApplicationRunner setup() {
		return args -> {
			Account accountUser = new Account().setFirstName("Petar").setLastName("Peric").setUsername("pera").setPassword("pera123").setRoles(Set.of(Role.USER));
			Account accountAdmin = new Account().setFirstName("Admin").setLastName("Adminic").setUsername("admin").setPassword("admin123").setRoles(Set.of(Role.ADMIN));
